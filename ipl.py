...
def totalTeams():
    ...
    return json.dumps(teams_dict, cls=NpEncoder)
...
def batsmanRecord(batsman, df):
    ...
    # Replace NaN with 0 if highest_score is NaN
    if pd.isnull(highest_score):
        highest_score = 0
...
bowler_data['bowler_run'] = bowler_data[[
    'extra_type', 'total_run']].apply(bowlerRun, axis=1)
...
bowler_data['isBowlerWicket'] = bowler_data[[
    'kind', 'isWicketDelivery']].apply(bowlerWicket, axis=1)
